  ...
  useEffect(() => {
    ...
    const setupLayers = () => {
          ...
          return new Promise((resolve) => {
            ...
            map.current.loadImage(path, (error, img) => {
              if (error || !img || !map.current) {
                console.warn(
                  `Failed to load ${iconName} icon from ${path}, using circle fallback`
                );
      ...
      const createPlaneLayers = () => {
        ...
        const useIcons =
          iconsLoaded.current.leader && iconsLoaded.current.follower;
...
<<<<<<< HEAD
        // Add heatmap layer before route layers so it renders behind everything
        const beforeLayerId = map.current.getLayer("route-a-line")
=======
        const beforeId = map.current.getLayer("route-a-line")
>>>>>>> main
          ? "route-a-line"
...
>>>>>>> main
  ...
  useEffect(() => {
    ...
    const allPoints = [
      ...selectedScenario.leader.points,
      ...selectedScenario.follower.points,
    ];
  ...
  useEffect(() => {
    ...
    if (!optimalDepartureData || !optimalDepartureData.path) {
      ...
      if (map.current.getSource("original-departure-path")) {
        map.current.getSource("original-departure-path").setData({
          type: "Feature",
          geometry: { type: "LineString", coordinates: [] },
        });
      }
      // Remove any partner-flight layers/sources from a previous run
      try {
        ...
        layers
          .filter((l) => l?.id?.startsWith("layer-partner-flight-"))
          .forEach((l) => {
        ...
        Object.keys(sources)
          .filter((id) => id.startsWith("partner-flight-"))
          .forEach((id) => {
            if (map.current.getSource(id)) {
    ...
    if (originalCoordinates.length > 0) {
      const originalSource = map.current.getSource("original-departure-path");
      if (originalSource) {
        const geoJsonData = {
          type: "Feature",
          geometry: {
          ...
          properties: {
            type: "original",
            origin: optimalDepartureData.route.origin,
            destination: optimalDepartureData.route.destination,
            scheduledDeparture: optimalDepartureData.route.scheduled_departure,
          },
        };
    ...
    if (optimalCoordinates.length > 0) {
      ...
      optimalSource.setData({
        ...
        properties: {
          ...
          origin: optimalDepartureData.route.origin,
          destination: optimalDepartureData.route.destination,
          optimalDeparture: optimalDepartureData.route.optimal_departure,
        },
      });
    ...
    partnerFlightIds.forEach((flightId) => {
      const flightPath = partnerFlightPaths[flightId];
  ...
  }, [optimalDepartureData, mapLoaded, styleLoaded]);
  ...
  useEffect(() => {
    ...
    const { leaderPosition, followerPosition, showConnector, phase } =
      replayState;
    ...
    if (leaderPosition) {
      ...
      const bearingDeg =
        leaderPosition.heading !== undefined
      ...
      map.current.getSource("leader-plane")?.setData({
        ...
        geometry: {
          type: "Point",
          coordinates: [leaderPosition.lon, leaderPosition.lat],
        },
        properties: {
          bearing: bearingDeg,
        },
    ...
    if (followerPosition) {
      ...
      const bearingDeg =
        followerPosition.heading !== undefined
      ...
      map.current.getSource("follower-plane")?.setData({
        ...
        geometry: {
          type: "Point",
          coordinates: [followerPosition.lon, followerPosition.lat],
        },
        properties: {
          bearing: bearingDeg,
        },
          ...
          <DropdownMenuTrigger asChild>
            ...
            >
              ...
              <span className="capitalize font-mono text-xs">
                {currentStyle.replace("satellite", "sat")}
              </span>
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent
        ...
        >
          <div className="p-4">
            ...
            <div className="space-y-4">
              ...
              <div
                className={`h-px ${isDark ? "bg-white/10" : "bg-slate-200"}`}
              />
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                >
                  ...
                  >
                    {popupData.metrics.formationMinutes} min
                  </span>
                </div>
                <div
                  className={`flex items-center gap-2 ${
                ...
                >
                  ...
                  >
                    {formatDistance(popupData.metrics.formationDistanceKm)}
                  </span>
                </div>
                <div
                  className={`flex items-center gap-2 ${
                ...
                >
                  ...
                  <span className="font-mono font-medium">
                    {formatCO2(popupData.metrics.co2SavedKg)}
                  </span>
                </div>
                <div
                  className={`flex items-center gap-2 ${
                ...
                >
                  ...
                  <span className="font-mono font-medium">
                    {formatNumber(popupData.metrics.fuelSavedKg)} kg
                  </span>
                </div>
              </div>
